...
app.post('/household', async (req, res) => {
    const {
        household_id, ho_ten_chu_ho, so_ho_khau, dia_chi, phuong_xa, tinh_thanhpho, ghi_chu
    } = req.body;
...
app.put('/household/:id', async (req, res) => {
    try {
        ...
        if (!household_id && !ho_ten_chu_ho && !so_ho_khau && !dia_chi && !phuong_xa && !tinh_thanhpho && !ghi_chu) {
...
app.post('/person', async (req, res) => {
    const {
        person_id, household_id, ho_ten, bi_danh, ngay_sinh, noi_sinh, dan_toc, nghe_nghiep, so_cccd, ngay_cap,
        noi_cap, ngay_dang_ky, dia_chi_truoc, quan_he_chu_ho, ghi_chu
    } = req.body;

    let pool;

    if (!person_id) {
        return res.status(400).json({ message: 'Thiếu person_id' });
    }
    ...
    try {
        ...
        request.input('person_id', person_id);
        request.input('household_id', household_id);
        ...
        request.input('so_cccd', so_cccd || null);
        request.input('ngay_cap', ngay_cap || null);
        request.input('noi_cap', noi_cap || null);
        request.input('ngay_dang_ky', ngay_dang_ky || null);
        request.input('dia_chi_truoc', dia_chi_truoc || null);
...
app.put('/person/:id', async (req, res) => {
    try {
        ...
        // Nếu không có trường nào để cập nhật
        if (!person_id && !household_id && !ho_ten && !bi_danh && !ngay_sinh && !noi_sinh && !dan_toc && !nghe_nghiep
            && !so_cccd && !ngay_cap && !noi_cap && !ngay_dang_ky && !dia_chi_truoc && !quan_he_chu_ho && !ghi_chu) {
            return res.status(400).json({ message: 'Cần ít nhất một trường để cập nhật.' });
        }
        ...
        const updates = [];
        if (person_id) updates.push('person_id = @person_id');
        if (household_id) updates.push('household_id = @household_id');
        ...
        if (ngay_cap) updates.push('ngay_cap = @ngay_cap');
        if (noi_cap) updates.push('noi_cap = @noi_cap');
        if (ngay_dang_ky) updates.push('ngay_dang_ky = @ngay_dang_ky');
        if (dia_chi_truoc) updates.push('dia_chi_truoc = @dia_chi_truoc');
        if (quan_he_chu_ho) updates.push('quan_he_chu_ho = @quan_he_chu_ho');
        if (ghi_chu) updates.push('ghi_chu = @ghi_chu');

        updateQuery += updates.join(', ');
        updateQuery += ' WHERE person_id = @id';

        const request = pool.request().input('id', id);

        if (person_id) request.input('person_id', person_id);
        if (household_id) request.input('household_id', household_id);
        ...
        if (ngay_cap) request.input('ngay_cap', ngay_cap);
        if (noi_cap) request.input('noi_cap', noi_cap);
        if (ngay_dang_ky) request.input('ngay_dang_ky', ngay_dang_ky);
        if (dia_chi_truoc) request.input('dia_chi_truoc', dia_chi_truoc);
...
app.delete('/person/:id', async (req, res) => {
    ...
    try {
        ...
        // 1. Thêm tham số ID
        request.input('personId', personIdentifier);
